...
class MetadataStore:
    ...
    def delete_by_doc_id(self, doc_id: str) -> int:
        ...
        # Get chunk IDs
        chunk_ids = cursor.execute("SELECT id FROM chunks WHERE doc_id = ?", (doc_id,)).fetchall()
    ...
    def update_chunk_content(self, chunk_id: int, content: str) -> bool:
        ...
        # Update FTS5
        cursor.execute("DELETE FROM chunks_fts WHERE rowid = ?", (chunk_id,))
        cursor.execute("INSERT INTO chunks_fts(rowid, content) VALUES (?, ?)", (chunk_id, content))
