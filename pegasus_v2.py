...
class MetadataStore:
    ...
    def insert_chunk(self, chunk: Dict[str, Any]) -> Optional[int]:
        ...
        try:
            ...
            # Add to FTS5 index
            cursor.execute(
                "INSERT INTO chunks_fts(rowid, content) VALUES (?, ?)", (chunk_id, chunk["content"])
            )
    ...
    def search_fts(
        self, query: str, k: int = 10, corpus: Optional[str] = None
    ) -> List[Dict[str, Any]]:
    ...
    def delete_corpus(self, corpus: str) -> int:
        ...
        # Get chunk IDs
        chunk_ids = cursor.execute("SELECT id FROM chunks WHERE corpus = ?", (corpus,)).fetchall()
...
class SearchEngine:
    ...
    def hybrid_search(
        ...
        for chunk_id, result in all_chunks.items():
            ...
            rrf_score = alpha * (1.0 / (rrf_constant + vector_rank)) + (1 - alpha) * (
                1.0 / (rrf_constant + keyword_rank)
            )
...
class Pegasus:
    ...
    def ingest(
        ...
        with self._lock:
            for doc in docs:
                ...
                # Ingest chunks
                for chunk_idx, (chunk_text_content, embedding) in enumerate(
    ...
    def search(
        ...
        *,
        ...
        with self._lock:
            if mode == "keyword":
                return self.search_engine.keyword_search(
                    query, k=k, corpus=corpus, filter_fn=filter_fn
                )
            elif mode == "hybrid":
                ...
                return self.search_engine.vector_search(
                    query, k=k, corpus=corpus, filter_fn=filter_fn
                )
